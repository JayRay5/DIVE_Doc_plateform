...
class TestProcessor:
    ...
    def test_processor_resizing_image(self, processor, image_size):
        ...
        inputs = processor(
            text="Test", images=fake_image, return_tensors="pt", padding=True
        )
        ...
        # Dim check
        assert (
            inputs["attention_mask"].shape[1] == inputs["input_ids"].shape[1]
        )  # sequence length
        assert (
        ...
        assert (
            inputs["pixel_values"].shape[2] == processor.image_processor.size["height"]
        )  # match target height
        assert (
            inputs["pixel_values"].shape[3] == processor.image_processor.size["width"]
        )  # match target width
