...
class PamConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        sequence_mapping_layer_type: Literal[
            "linear_projection", "bilinear_interpolation"
        ...
        **kwargs,
        ...
        if isinstance(self.encoder_config, dict):
            ...
            if encoder_config["model_type"] == "donut-swin":
                ...
                self.encoder_config = CONFIG_MAPPING[encoder_config["model_type"]](
                    **encoder_config
                )
        ...
        if isinstance(self.pam_config, dict):
            ...
            if pam_config["model_type"] == "pam":
                ...
                raise ValueError(
                    f"pam_config['model_type'] should be 'pam', got {pam_config['model_type']}"
                )
        """
...
class DIVEdocConfig(PretrainedConfig):
    ...
    sub_configs = {
        "vision_config": SwinPamVisionEncoderConfig,
        ...
        **kwargs,
        ...
        if isinstance(self.vision_config, dict):
            ...
            if vision_config["model_type"] == "swinpam":
                self.vision_config = SwinPamVisionEncoderConfig(
                    encoder_config=vision_config["encoder_config"],
                    pam_config=vision_config["pam_config"],
                )
            else:
        ...
        self.text_config.num_image_tokens = (
            self.vision_config.pam_config.teacher_fmap_dim[0]
...
def get_vision_config(
    visual_encoder_type: Literal["swinpam", "siglip80m"],
    ...
    if visual_encoder_type == "swinpam":
        encoder_config = get_swin_vision_config(
            image_size=image_size, hidden_size=student_embedding_dim
        )
        ve_config = SwinPamVisionEncoderConfig(
            encoder_config=encoder_config, pam_config=pam_config
        )
        return ve_config

    else:
        raise ValueError(
            f"Unknown visual encoder type, need 'swinpam', got {visual_encoder_type}."
        )
